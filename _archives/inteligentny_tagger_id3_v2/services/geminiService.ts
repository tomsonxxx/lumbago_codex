...
const handleGeminiError = (error: unknown): Error => {
    if (error instanceof Error) {
        ...
        if (msg.includes('api key not valid')) {
            return new Error("Klucz API Gemini jest nieprawidłowy. Sprawdź go w ustawieniach.");
        }
        if (msg.includes('429') || msg.includes('resource has been exhausted') || msg.includes('rate limit')) {
            return new Error("Przekroczono limit zapytań do AI. Odczekaj chwilę i spróbuj ponownie.");
        }
        if (msg.includes('failed to fetch') || msg.includes('network')) {
             return new Error("Błąd sieci. Sprawdź swoje połączenie z internetem.");
        }
        // Return a slightly cleaned-up version of the original error
        return new Error(`Błąd AI: ${error.message}`);
    }
    return new Error("Wystąpił nieznany błąd podczas komunikacji z AI.");
};
