...
from concurrent.futures import ThreadPoolExecutor, as_completed, wait as _futures_wait, FIRST_COMPLETED
import hashlib
import html
import json
import re
...
from core.audio import extract_metadata, read_tags
...
class MetadataEnricher:
    ...
    def enrich_from_acoustid(self, track: Track) -> Track | None:
        ...
        from services.metadata_providers import _best_mbid_from_acoustid
...
class AutoMetadataFiller:
    ...
    def _run_online_sources_parallel(
        self,
        enricher: MetadataEnricher,
        track: Track,
        ...
        try:
            mb_future = pool.submit(self._collect_musicbrainz_snapshot, enricher, track)
            futures[mb_future] = "musicbrainz"
            ...
            for f in done:
                source_id = futures[f]
                try:
                    self._apply_snapshot(f.result(), source_id, track, report)
                except Exception:
                    pass

            # Slow pass — wait for laggards up to SLOW_TIMEOUT_S, fill remaining empty fields
            if pending:
                done2, still_pending = _futures_wait(pending, timeout=SLOW_TIMEOUT_S - FAST_TIMEOUT_S)
                for f in done2:
                    source_id = futures[f]
                    try:
                        self._apply_snapshot(f.result(), source_id, track, report)
                        ...
                        pass
                # Cancel truly stuck futures (best-effort)
                for f in still_pending:
                    f.cancel()
        finally:
            pool.shutdown(wait=False)

    def _collect_musicbrainz_snapshot(
        self, enricher: MetadataEnricher, track: Track
    ) -> tuple[str, Track | None, list[SourceProbe]]:
        probes: list[SourceProbe] = []
        probe_track = deepcopy(track)
        snapshot_report = MetadataFillReport(method="online")
        self._run_musicbrainz_source(enricher, probe_track, snapshot_report)
        return ("musicbrainz", probe_track, snapshot_report.sources)

    def _collect_portal_snapshot(
        self, track: Track
    ) -> tuple[str, Track | None, list[SourceProbe]]:
        probe_track = deepcopy(track)
        snapshot_report = MetadataFillReport(method="online")
        self._run_portal_search_sources(probe_track, snapshot_report)
        return ("portals", probe_track, snapshot_report.sources)

    def _apply_snapshot(
        self,
        snapshot: tuple[str, Track | None, list[SourceProbe]],
        source_id: str,
        track: Track,
        report: MetadataFillReport,
    ) -> None:
        _, probe_track, probes = snapshot
        report.sources.extend(probes)
        if probe_track is None:
            return
        _copy_missing_fields(track, probe_track, set(SEARCHABLE_METADATA_FIELDS))
