    ...
    jsmediatags.read(file, {
      onSuccess: (tag: any) => {
        ...
        if (tagData.picture) {
            ...
            // Fix: Some MP4 tags don't return a MIME type in 'format'. 
...
const applyMP4TagsToFile = async (fileBuffer: ArrayBuffer, tags: ID3Tags): Promise<ArrayBuffer> => {
    ...
    // Wrapped in try-catch blocks to prevent writer crashes on unsupported structures.
    if (tags.mood) {
        try {
    ...
    if (tags.albumCoverUrl) {
         try {
            ...
            // Non-fatal error for cover art, proceed with other tags
        }
    }
