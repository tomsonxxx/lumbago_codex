...
Write-Host "Portable artifact for clean Windows P1 (per docs/clean_windows_test.md + SZPIEG research 2026-06-25 + Analyzer + Plan 'nowa lista' (luki w smoke/build) + Etap4 playback reliability + finalny efekt końcowy (VLC prio, visible '⚠ Audio niedostępne' + 'Pobierz VLC z videolan.org', diagnostics, targeted updates, file=load vs stream=transport, guards, EFFECT, booth-visible states, portable notes) — must document identical). For full DJ audio after extract on clean machine: Pobierz VLC z videolan.org or unpack portable VLC next to LumbagoMusicAI.exe + restart. Local smoke + scripts provide full coverage of portable exe run + resources(strict); full manual (import 1-3, detail, player load/play/cue/hotcue, APPDATA DB/settings) + clean-VM test pending per original plan + 'nie przestawaj'."