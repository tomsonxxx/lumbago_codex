...
Write-Host "Portable structure note (ties to clean_windows_test.md + SZPIEG Build Spec): exe present, resources checked (fpcalc, ui/assets, docs, icons, themes)."
Write-Host "Full functional manual on clean/VM: import 1-3 audio, library appear, detail edit+save, DJ Player open/load (drag or button) 1-2 tracks, play/seek/hotcue (set/jump 8/deck if dual)/loop/crossfader/waveform/status, no crash unscanned, %APPDATA%\LumbagoMusicAI\lumbago.db + settings.json creation."
Write-Host "VLC guidance ('Pobierz VLC z videolan.org' or portable next to exe), backend diagnostics."
