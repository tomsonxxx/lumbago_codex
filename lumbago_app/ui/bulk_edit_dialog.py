...
class BulkEditDialog(QtWidgets.QDialog):
    ...
    def _build_ui(self):
        ...
        count_label = QtWidgets.QLabel(f"Zostanie zmienionych: {len(self._tracks)} utworów")
        count_label.setStyleSheet("color: #9aa6b2;")
        ...
        # Genre with append mode
        genre_widget = QtWidgets.QWidget()
        genre_row = QtWidgets.QHBoxLayout(genre_widget)
        genre_row.setContentsMargins(0, 0, 0, 0)
        self.genre_input = QtWidgets.QLineEdit()
        self.genre_append_check = QtWidgets.QCheckBox("Dołącz")
        self.genre_append_check.setToolTip("Dodaj do istniejącej wartości zamiast nadpisywać")
        genre_row.addWidget(self.genre_input, 1)
        genre_row.addWidget(self.genre_append_check)

        form.addRow(self.artist_check, self.artist_input)
        form.addRow(self.album_check, self.album_input)
        form.addRow(self.genre_check, genre_widget)
    ...
    def _apply(self):
        for track in self._tracks:
            ...
            if self.genre_check.isChecked():
                ...
                if self.genre_append_check.isChecked() and track.genre:
                    track.genre = f"{track.genre}, {new_genre}" if new_genre else track.genre
                else:
