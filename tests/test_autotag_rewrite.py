...
def test_unified_autotagger_picks_best_candidate(monkeypatch):
    ...
    monkeypatch.setattr(service, "_search_theaudiodb", lambda _track: None)
    monkeypatch.setattr(service, "_search_listenbrainz", lambda _track: None)
...
def test_apply_best_match_updates_track():
    ...
    best_candidate = Candidate(source="MusicBrainz", score=90, title="New Title", artist="New Artist", album="New Album", genre="Trance", bpm=128.0, key="8A", mood="energetic")
    result = type("R", (), {"candidates": [best_candidate], "best_match": best_candidate})()
