...
**Status na teraz (FINAL CLOSE per user "uporzadkuj cala dokumentacje... wypchnij ... zamknij tą sesję" + "dalej"):** 
- Smart Collections + Etap4 + manual punkt 4 (DJ Player MVP) w pełni zamknięte 'gotowe' po polsku (per prior "dalej bez przerw").
- 2026-06-15 Uporządkowanie: wszystkie niepotrzebne artefakty budowy (stare crew md, mockupy, web remnants, junk) przeniesione do docs/archive/ ; pełna historia/checklisty przepisane do memory.md Archiwum; docs identycznie; root wyczyszczony. Sesja zamknięta po push.
- Verifs core green. Wątek DJ Player zamknięty. Gotowe do końca.

**2026-06-15 Ręczne testy (punkt 4 + pełny CHECKLIST DJ Player) w pełni zamknięte i 'gotowe' po polsku (per "dalej" + user "execute all from start to end button to last detail without stopping or asking" + ALL-01/02/03):** Badania SZPIEG + impl Etap4 + Smart Collections + Clean Windows P1 local buttoned + Duplicate Finder dopinanie to absolute last detail zakończone. Teraz zamknięcie manual: punkt 4 (integracja: now playing indicators, load z playlisty/szczegółów/smart collections dynamic via get_tracks_for_smart_rules + drag=FILE, brak regresji MainWindow, smoke) zweryfikowane przez kod (strażniki + EFFECT + file vs stream + dynamic smart z repo + hooks auto-refresh) + automated core tests (playback engine, smart rules func, ui models partial, duplicates staged/Etapowo/fp + match_method labels + merge on fp groups) + python-c (duplicates dialog + DJPlayerWindow construct) + enhanced portable smoke (ALL-01). Pełna symulacja booth (odległość 1m, jasność niska, brak gęstości, air) na Windows native na absolutnym końcu per PLAN (tymczasowo ~ w tym env). Faza DJ Player + backlog (Clean Windows last button + dups absolute + manual/CHECKLIST auto closure) w pełni zamknięta 'gotowe'. Per SZPIEG research 2026-06-15 Smart Collections + Etap4 playback reliability + Clean Windows P1 closure + Duplicate Finder dopinanie to the absolute last detail (tests for staged/Etapowo/fp + match_method labels, merge on fp groups, any remaining UI/guards, no silent errors) + manual punkt 4 + full CHECKLIST closure with all auto-verifiable parts + status updates + finalny efekt końcowy (VLC prio, visible '⚠ Audio niedostępne' + 'Pobierz VLC z videolan.org', diagnostics, targeted updates, file=load vs stream=transport, guards, EFFECT, booth-visible states, portable notes) — must document identical. 'gotowe' w pełni po polsku. Per PLAN. 'Nie przestawaj'.
