...
Obecny kod jest **w pełni desktopową aplikacją PyQt6 (desktop-only focus)** z bogatymi funkcjami domenowymi DJ. **Web MVP (SPA React + FastAPI...)** jest legacy / standalone (tylko ID3 tagger) i nie jest rozwijany dla głównego produktu.

Największa rozbieżność (historyczna):
- spec opisuje **SPA React + FastAPI + PostgreSQL + Redis + Celery + S3/MinIO**, 
- repo implementuje dziś **desktopową aplikację PyQt6 + lokalne SQLite + bezpośredni dostęp do repo (bez REST dla głównej app)**.

**Pełna migracja DJ** do sole new arch (ui/dj/* + dj_player_window.py z hardcoded True; usunięto stare Deck/SinglePlayerView) + robust None handling w beatgrid/auto_cue + crash fixes w DJ/analysis — wszystko w desktop.

Innymi słowy: **desktop jest jedyną i aktualną platformą**; web to historyczny MVP.
...
### 2.1 Web MVP vs desktop app (current sole focus)
Spec jasno mówi o aplikacji **webowej SPA (React + FastAPI)** — to był historyczny MVP. Obecny projekt jest w pełni **desktopową aplikacją Python/Windows z PyQt6** (jedyną wspieraną).

**Skutek praktyczny:** desktop PyQt6 jest jedyną implementacją; web/legacy nie jest używany dla DJ flows, library, analysis itp. Web backend/frontend zachowany tylko dla standalone ID3 tagger.
